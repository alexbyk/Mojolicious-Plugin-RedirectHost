...
Mojolicious::Plugin::RedirectHost - Перенаправляет запросы с зеркальных доменов на основной (полезно для SEO, чтобы не расплывались показатели ТИЦ и PageRank)
