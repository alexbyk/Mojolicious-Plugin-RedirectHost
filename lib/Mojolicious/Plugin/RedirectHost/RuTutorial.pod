...
Если путь запроса будет совпадать с C<except_path>, редиректа не будет
Путь должен начинаться с лидирующего '/';
Полезно, чтобы избежать /robots.txt редиректов
В будущем, возможно я сделаю это более гибким
